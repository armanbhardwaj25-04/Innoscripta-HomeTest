...
const RectangularCarousel: React.FC<{ articles: Article[] }> = React.memo(
  ({ articles }) => {
...
const CircularCarousel: React.FC<{
  ...
  heading?: string;
  noImage?: boolean;
}> = React.memo(({ data = [], heading, noImage }) => {
  return data.length ? (
      ...
      >
        {data.map((source, index) => (
          <SwiperSlide key={source.id || index} style={{ width: "auto" }}>
            <Link
              to={`/${heading?.toLowerCase()}/${encodeURIComponent(source.name?.toLowerCase() || "")}`}
            ...
            >
              {!noImage ? (
                ...
                <div className="source-logo no-image">
                  <p>{source.name}</p>
                </div>
              )}
...
const NewsCarousel: React.FC<NewsCarouselProps> = ({
  ...
  const validArticles =
    articles && Array.isArray(articles) ? articles : FALLBACK_CAROUSEL;
  ...
  if (version === CIRCULAR_CAROUSEL_TYPE) {
    return (
      <CircularCarousel data={validData} heading={heading} noImage={noImage} />
    );
  }
  return null;
};
